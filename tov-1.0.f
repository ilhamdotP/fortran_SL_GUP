...
C        IL=FIXEDIL
       ...
       CNST2=(EDC+PC)*(1.D0-DEDPC+2.D0*DSDPC)
       LMDCC=1.D0/3.D0*(-2.D0*CNST2*GS*PI*XC*XC*
     &      (3.D0*PC*PC+4.D0*PC*EDC+EDC*EDC)) 
       IK=0
...
C       WRITE(*,*)ALPCC,LMDCC,ALPHA,LAMBDA
      IF (ABS(ALPHA).GT.1.D-9) THEN
            PRINT *,"PRINT 1"
            ALPCC=ALPCC-ALPHA
            GOTO 29
      ENDIF
      IF (ABS(LAMBDA).GT.1.D-9) THEN
            PRINT *,"PRINT 2"
            LMDCC=LMDCC-LAMBDA
            GOTO 29
      ENDIF
...
C             PRINT *,"IK=",IK
      ...
      IF (IK.EQ.1) THEN 
            PRINT *,"PRINT 4"
            WRITE(*,*)ALPCC,LMDCC,ALPHA,LAMBDA
...
c--------------------------------------------------------------
     ...
     &       -2.D0*SIGMA/XA
      ...
      EK(J,2)=H/MSS*(4.D0*PI*XA*XA*EDENT) 
      
      EK(J,3)=H*ALPHAP
      
      EK(J,4)=H*(PRESSP*((EDEN+PRESS)
     &       *(1.D0-DEDP(PRESS)+2.D0*DSDP(PRESS))
