...
C        OPEN (unit=2,STATUS='unknown',FILE='CatatanB145SC.dat')
       OPEN (unit=3,STATUS='unknown',FILE='radmass_Y=-1.15_CT=1e7.dat')
       OPEN (unit=1,STATUS='unknown',FILE='profil_Y=-1.15_CT=1e7.dat')
...
C      &      LAMBDA*1.D-6,EDEN,SIGMA
     ...
     &      LAMBDA*1.D-6,EDEN,SIGMA
      ...
      IF (IK.EQ.0) THEN
            IKK=IKK+1
            WRITE(*,*)IL,(ALPHA-0.5D0*LOG(1.D0-2.D0*GS*MSS*Y(2)/XP))
     &       ,LAMBDA,IKK
